...
if not os.path.exists("tester-images"):
    os.mkdir("tester-images")

sleepTime = 0 # How long to sleep between rendering images

try:
  # Load the renderer
  eyeRenderer = CDLL("/home/n.dejong@ad.ufl.edu/compound-ray/build/make/lib/libEyeRenderer3.so")
  ...
  # Load a scene
  eyeRenderer.loadGlTFscene(c_char_p(b"/home/n.dejong@ad.ufl.edu/compound-ray/data/test-scene/moontalk.gltf"))
  ...
  #eyeTools.setRederSize(eyeRenderer, renderWidth, renderHeight)
  ...
  for i in range(5):
    ...
    eyeRenderer.saveFrameAs(c_char_p(("tester-images/test-image-"+str(i)+".ppm").encode()))
    ...
    if(eyeRenderer.isCompoundEyeActive()):
      ...
      eyeRenderer.saveFrameAs(c_char_p(("tester-images/test-image-"+str(i)+"-100samples.ppm").encode()))# Save it
